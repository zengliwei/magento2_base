...
define([
    ...
    return Element.extend({
        ...
        setInitialValue: function () {
            let value = this.getInitialValue().map(this.processFile, this);
        ...
        onBeforeFileUpload: function (e, data) {
            ...
            if (allowed.passed) {
                target.on('fileuploadsend', function (event, postData) {
                    if (!postData.data.has('param_name')) {
        ...
        onFileUploaded: function (e, data) {
            let uploadedFilename = data.files[0].name,
                file = data.result,
                error = file.error;
                ...
                _.each(this.aggregatedErrors, function (error) {
                    notification().add({
                        ...
                        insertMethod: function (constructedMessage) {
                            let escapedFileName = $('<div>').text(error.filename).html(),
